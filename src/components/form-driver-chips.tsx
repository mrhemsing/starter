...
export function FormDriverChips({ chips, limit = 3, compact = false }: { chips?: FormDriverChip[]; limit?: number; compact?: boolean }) {
  ...
  return (
    <div className={`flex max-h-14 flex-wrap gap-1.5 overflow-hidden ${compact ? "mt-2" : "mt-3"}`} aria-label="Form drivers">
      {shown.map((chip) => (
        <span
          key={`${chip.key}-${chip.direction}-${chip.label}`}
          className={`inline-flex items-center rounded border px-2 py-1 font-mono text-[10px] uppercase tracking-[0.1em] ${
            chip.direction === "good"

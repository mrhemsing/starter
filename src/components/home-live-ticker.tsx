  ...
  return (
    ...
    >
      <div className="flex min-h-11 items-center gap-3">
        ...
        <div className="min-w-0 flex-1 overflow-x-auto motion-reduce:overflow-x-auto">
          <div className="home-live-ticker-track flex w-max items-center gap-5 py-2 pr-5 motion-reduce:animate-none">
...
function TickerEntryItem({ entry, duplicate }: { entry: TickerEntry; duplicate: boolean }) {
  return (
    <a
      href={entry.href}
      className="flex shrink-0 items-center gap-1.5 whitespace-nowrap font-mono text-[11px] uppercase tracking-[0.13em] text-zinc-300 hover:text-amber-200"
      aria-hidden={duplicate ? "true" : undefined}
      tabIndex={duplicate ? -1 : undefined}
    >
      {entry.state === "upcoming" ? <span className="text-zinc-500">NEXT</span> : null}
      <span className="font-semibold text-zinc-50">{entry.label}</span>
      {entry.score !== undefined ? <span className="text-zinc-300">{Math.round(entry.score)}</span> : null}
...
function buildTickerEntries(board: LiveScoreboard | null): TickerEntry[] {
  ...
  const liveRows = board.rows
    .filter((row) => row.scoreLabel === "PROV" && row.gsPlus !== null)
  ...
  return [
    ...liveRows.map((row) => ({
      ...
      glyph: (row.gsPlus ?? 0) >= (row.projectedGsPlus ?? 50) ? "up" as const : "down" as const,
      state: "live" as const,
    })),

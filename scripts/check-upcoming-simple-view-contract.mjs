...
assert(simpleBoard.includes("text-left text-sm") && !simpleBoard.includes("lg:text-left"), "Simple context text must be left-aligned on mobile and desktop.");
...
assert(simpleBoard.includes("inline-flex w-full flex-col items-center justify-center") && simpleBoard.includes("sm:w-auto sm:flex-row sm:gap-1") && simpleBoard.includes("function SimpleFormChipLabel") && simpleBoard.includes("whitespace-nowrap"), "Simple form chips must span the mobile card width, stack label over value on mobile, and return to inline label/value at larger breakpoints.");
...
assert(simpleBoard.includes("hasQualifiedStarterFormSample(starter) ? starter.tier ?? null : null") && simpleBoard.includes("limitedSampleFormText(starter.rgs)") && simpleBoard.includes('data-simple-form-chip-source={formBand ? "heat-band" : starter.formStatus === "ok" ? "limited-sample" : starter.formStatus}'), "Simple cards must render limited-sample arms as neutral LTD chips, not FORM pseudo-bands.");
...
assert(detailedBoard.includes("hasQualifiedStarterFormSample(starter) && starter.rgs !== undefined && starter.tier") && detailedBoard.includes("<LimitedSampleFormChip value={starter.rgs}") && detailedBoard.includes("return hasQualifiedStarterFormSample(starter) && Boolean(starter.spark?.length && starter.tier);"), "Detailed Upcoming cards must gate band lines, sparklines, and limited labels through the shared LTD treatment.");
assert(watchlistPage.includes("hasQualifiedFormSummarySample(entry)") && watchlistPage.includes("<LimitedSampleFormChip value={entry.rgs}") && watchlistPage.includes("LIMITED_SAMPLE_FORM_COLOR"), "Watchlist cards must use the shared LTD chip and neutral color for limited-sample form slots.");
assert(liveScoreboard.includes("hasQualifiedStarterFormSample(starter)") && liveScoreboard.includes("<LimitedSampleFormChip value={starter.rgs}") && liveScoreboard.includes('{tier?.label ?? "Form"} {starter.rgs.toFixed(1)}'), "Live pregame cards must show band labels for qualified arms and LTD for limited-sample arms.");

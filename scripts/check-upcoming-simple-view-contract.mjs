...
assert(viewMode.includes('label="View"') && viewMode.indexOf('{ value: "detailed", label: "Detailed"') < viewMode.indexOf('{ value: "simple", label: "Simple"'), "Upcoming VIEW must render as DETAILED / SIMPLE segmented options.");
...
assert(viewMode.includes('window.localStorage.getItem(STORAGE_KEY) === "SIMPLE" ? "simple" : "detailed"'), "Stored SIMPLE preference must restore simple view while absent storage defaults to DETAILED.");
...
assert(viewMode.includes('const DEFAULT_VIEW_MODE: UpcomingViewMode = "detailed";') && viewMode.includes("return DEFAULT_VIEW_MODE;"), "Missing or blocked storage must default to DETAILED.");
...
assert(page.includes("data-upcoming-view-mode-init") && page.includes('window.localStorage.getItem("tts.upcoming.view") === "SIMPLE" ? "simple" : "detailed"'), "Upcoming page must set the stored view mode before the controls paint and default absent storage to DETAILED.");
...
assert(simpleBoard.includes("data-simple-header-band") && simpleBoard.includes("<span aria-hidden=\"true\" />") && simpleBoard.includes("data-upcoming-simple-score") && simpleBoard.indexOf("data-upcoming-simple-score") > simpleBoard.indexOf("data-simple-diagonal-panels"), "Simple header must contain rank and time only, with score moved out of the header.");
assert(simpleBoard.includes("grid-cols-[minmax(0,1fr)_86px_minmax(0,1fr)]") && simpleBoard.includes("data-simple-score-seam-column") && simpleBoard.includes("min-h-[150px] flex-col items-center justify-center") && simpleBoard.includes("text-[38px]") && simpleBoard.includes("data-simple-watch-score>{game.gameWatchScore.toFixed(1)}"), "Simple cards must render the watch score in a centered seam column between portrait cells.");
...
assert(headshot.includes("suppressThermalBackground") && headshot.includes('background: suppressThermalBackground ? "transparent" : thermalBackground(resolvedBand)') && simpleBoard.includes("suppressThermalBackground"), "Simple fight-card headshot containers must suppress the inline thermal background gradient.");
...
assert(liveScoreboard.includes("hasQualifiedStarterFormSample(starter)") && liveScoreboard.includes("<FormValueWhisperLine value={starter.rgs}") && liveScoreboard.includes("data-live-pregame-form-line"), "Live pregame cards must show band labels for qualified arms and LTD for limited-sample arms through the shared form line.");

...
assert(simpleBoard.includes("grid-cols-[minmax(0,1fr)_86px_minmax(0,1fr)]") && simpleBoard.includes("data-simple-score-seam-column") && simpleBoard.includes("min-h-[150px] flex-col items-center justify-center") && simpleBoard.includes("text-[38px]") && simpleBoard.includes("data-simple-watch-score>{scoreLabel}</p>") && simpleBoard.includes('const scoreLabel = hasNamedStarterMatchup ? game.gameWatchScore.toFixed(1) : "--";'), "Simple cards must render the watch score in a centered seam column between portrait cells and dash TBD matchups.");

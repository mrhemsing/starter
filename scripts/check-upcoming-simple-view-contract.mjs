...
const watchRank = await readFile("src/lib/watch-rank.ts", "utf8");
...
assert(types.includes("watchRank: number;") && types.includes("watchRankOf: number;"), "Upcoming matchup data must carry stable watch rank and slate-size fields.");
assert(tonightService.includes('import { assignWatchRanks } from "@/lib/watch-rank";') && tonightService.includes("const rankedGames = assignWatchRanks") && watchRank.includes("export function assignWatchRanks") && watchRank.includes("b.gameWatchScore - a.gameWatchScore") && watchRank.includes("a.firstPitch.localeCompare(b.firstPitch)") && watchRank.includes("a.away.localeCompare(b.away)") && watchRank.includes("watchRankOf"), "Slate construction must assign watch rank once by Hook score with deterministic first-pitch and away-team tiebreaks.");
assert(page.includes("a.watchRank - b.watchRank") && page.includes("sortMode={controls.sort}") && detailedBoard.includes('sortMode = "watch"') && detailedBoard.includes('sortMode === "watch" ? shownGames.find((game) => game.watchRank === 1) : undefined'), "Upcoming sorting must use stable watch rank and bind hero selection to the URL-backed sort mode.");
assert(detailedBoard.includes('data-watch-rank-gold={game.watchRank === 1 ? "true" : undefined}') && detailedBoard.includes("game.watchRankOf") && detailedBoard.includes("game.watchRank") && !detailedBoard.includes("index + 2"), "Detailed cards must read all watch-rank presentation from matchup data and expose one gold rank-one treatment.");
...
assert(page.includes("sortMode={controls.sort}") && simpleBoard.includes('data-simple-sort-mode={sortMode}'), "Simple boards must receive the active sort mode.");
assert(simpleBoard.includes('data-simple-rank-visible={String(showRankSlot)}') && simpleBoard.includes('const showRankSlot = sortMode === "watch";') && simpleBoard.includes('const rankLabelText = hasNamedStarterMatchup ? `#${game.watchRank}` : "--";') && simpleBoard.includes("data-simple-card-rank") && simpleBoard.includes("function hasNamedStarters"), "Simple VS rank slots must render stable watchRank data only in Watch Rank sort and dash TBD matchups.");
...
assert(simpleBoard.includes("data-simple-name-band-label") && simpleBoard.includes('data-simple-name-band-label-align="bottom"') && simpleBoard.includes("simpleStarterBandLabel(starter, formBand)") && simpleBoard.includes("mt-[6px] inline-flex items-end gap-2") && simpleBoard.includes("text-[12px] uppercase leading-none tracking-[0.12em] text-zinc-500") && simpleBoard.includes('className="whitespace-nowrap leading-none" style={{ color: heatColor }}'), "Simple starter names must leave 6px above the colored heat-band whisper label, with the mobile score and label bottom-aligned without wrapping.");
assert(simpleBoard.includes("const href = qualifiedSample && starter.pitcherId && starter.name ? pitcherHref") && detailedBoard.includes("function starterProfileHref(starter: TonightStarter)") && detailedBoard.includes("!hasQualifiedStarterFormSample(starter)"), "Upcoming must only link pitcher names when enough starter-form data exists to render the destination profile.");
...
assert(simpleBoard.includes('data-simple-card-rank-tone={isTopWatchRank ? "gold" : hasNamedStarterMatchup ? "ranked" : "muted"}') && simpleBoard.includes('isTopWatchRank ? "text-amber-300" : hasNamedStarterMatchup ? "text-white" : "text-zinc-400"') && simpleBoard.includes('data-watch-rank-gold={isTopWatchRank ? "true" : undefined}'), "Simple headers must render exactly the stable number-one watch rank in gold while other named ranks stay white and TBD ranks stay gray.");
...
assert(limitedSampleChip.includes("export function FormValueWhisperLine") && limitedSampleChip.includes("formBandWhisperLabel") && limitedSampleChip.includes("formLineEraText") && limitedSampleChip.includes("L${window} ERA") && limitedSampleChip.match(/style=\{\{ color: valueColor \}\} data-form-band-whisper/g)?.length === 2, "Colored value and heat-band whisper plus neutral Ln ERA must be one shared component in stacked and inline layouts.");
...
assert(liveScoreboard.includes('import { UpcomingSimpleCard, UpcomingSimpleCardGrid } from "@/components/upcoming-simple-board";') && liveScoreboard.includes('data-live-pregame-simple-card="true"') && liveScoreboard.includes('<UpcomingSimpleCardGrid data-live-pregame-first-up-grid="true" data-live-pregame-first-up-count={firstUpGames.length}>') && liveScoreboard.includes('<UpcomingSimpleCard key={firstUpGame.gamePk} game={firstUpGame} leagueMeanGS={slate.leagueMeanGS} rankLabel={slate.headerLabel.toLowerCase()} sortMode="time" />'), "Live pregame cards must reuse the shared Simple card while hiding rank numerals in time order.");

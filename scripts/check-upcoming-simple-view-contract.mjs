...
assert(liveScoreboard.includes('import { UpcomingSimpleCard, UpcomingSimpleCardGrid } from "@/components/upcoming-simple-board";') && liveScoreboard.includes('data-live-pregame-simple-card="true"') && liveScoreboard.includes('<UpcomingSimpleCardGrid data-live-pregame-first-up-grid="true" data-live-pregame-first-up-count={firstUpGames.length}>') && liveScoreboard.includes('<UpcomingSimpleCard key={firstUpGame.gamePk} game={firstUpGame} leagueMeanGS={slate.leagueMeanGS} rankLabel={slate.headerLabel.toLowerCase()} />'), "Live pregame cards must reuse the shared Simple card and stable rank data so band labels and LTD limited-sample labels stay aligned with Upcoming.");

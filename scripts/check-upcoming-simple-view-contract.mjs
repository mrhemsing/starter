...
assert(simpleBoard.includes("justify-center gap-3") && simpleBoard.includes("sm:gap-4") && simpleBoard.includes("lg:gap-5"), "Simple board must keep mobile breathing room between cards while preserving larger breakpoint spacing.");
...
assert(simpleBoard.includes("data-simple-name-band-label") && simpleBoard.includes('data-simple-name-band-label-align="bottom"') && simpleBoard.includes("simpleStarterBandLabel(starter, formBand)") && simpleBoard.includes("inline-flex items-end gap-2") && simpleBoard.includes("text-[12px] uppercase leading-none tracking-[0.12em] text-zinc-500"), "Simple starter names must show the heat-band whisper label underneath the pitcher name with the mobile score and label bottom-aligned.");
...
assert(liveScoreboard.includes('import { UpcomingSimpleCard } from "@/components/upcoming-simple-board";') && liveScoreboard.includes('data-live-pregame-simple-card="true"') && liveScoreboard.includes('<UpcomingSimpleCard game={game} rank={1} leagueMeanGS={slate.leagueMeanGS} rankLabel={slate.headerLabel.toLowerCase()} sortMode="time" />'), "Live pregame cards must reuse the shared Simple card so band labels and LTD limited-sample labels stay aligned with Upcoming.");

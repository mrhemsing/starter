...
assert(simpleBoard.includes('data-simple-starter-card-back-source="flat"') && !simpleBoard.includes("style={{ background: simpleStarterHeatZoneGradient(panelColor, align, \"bottom\") }}"), "Simple starter bottom stat bar must remove the old background gradient.");
...
assert(simpleBoard.includes("data-simple-mini-stat-line") && simpleBoard.includes("text-[9px]") && simpleBoard.includes("lg:text-white"), "Simple desktop mini-stat text, including Proj GS+, must render at 9px and white on desktop.");
...
assert(simpleBoard.includes("<FormValueWhisperLine") && simpleBoard.includes("value={starter.rgs}") && simpleBoard.includes("era={starter.seasonStats?.era}") && simpleBoard.includes("qualifiedSample={Boolean(formBand)}") && simpleBoard.includes('valueClassName="text-[12px]"') && simpleBoard.includes("eraOnNewLine"), "Simple form line must render a 12px colored value, whisper band, and forced-break L5 ERA through the shared component.");
...
assert(simpleBoard.includes("const SIMPLE_EVEN_PANEL_COLOR") && simpleBoard.includes("starterHeatPanelColor") && simpleBoard.includes("data-simple-starter-panel-source=\"three-band\"") && simpleBoard.includes("data-simple-form-line-color={heatColor}") && simpleBoard.includes("HEAT_BANDS.find"), "Simple starter heat zones must use shared heat-band color tokens.");
